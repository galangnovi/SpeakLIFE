...
export default function EmergencyDialog({ open, onClose }: EmergencyDialogProps) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    await fetch("https://99c45a97fb09.ngrok-free.app/webhook/emergency", {
      method: "POST",           
      headers: {
        ...
        {!success ? (
          <div>
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
              ...
              <DialogFooter>
                <Button type="submit" disabled={loading} className="bg-green-500 hover:bg-green-400 cursor-pointer">
          ...
          <div className="text-center space-y-4">
            <p className="text-green-600 font-semibold">
              ✅ Terima kasih, data kamu sudah diterima. Kami akan segera membantu masalahmu
            </p>
            <Button onClick={onClose} className="cursor-pointer">Tutup</Button>
          </div>
        )}
      </DialogContent>
    </Dialog>
